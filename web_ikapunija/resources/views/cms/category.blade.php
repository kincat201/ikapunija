            ...
            <div class="modal-body">
                <form id="form_submit"> 
                    ...
                    <div class="form-group ves-custom" id="ves_type">
                        <label class="d-flex">Jenis Category <div class="cus_req" id="req_type">&nbsp*</div></label>
                        <select class="form-control select2 form-custom" id="form_type" style="width: 100%;">
                            <option value="">- Pilih Category -</option>
                            <option value="all"> Semua </option>
                            <option value="agenda"> Agenda </option>
                            <option value="berita"> Berita Alumni </option>
                            <option value="loker"> CDC </option>
                            <option value="announce"> Pengumuman </option>
                        </select>
                    </div>
                    <div class="form-group ves-custom" id="ves_nama">
                        <label class="d-flex">Nama Category <div class="cus_req" id="req_nama">&nbsp*</div></label>
...
<script>
    ...
    function addData() {
       ...
       $.ajax({
            type: "POST",
            url: '<?php echo env('APP_URL').'api/addCategory'; ?>',
            headers: {
    ...
    function getDetail(obj) {
        ...
        $.ajax({
            type: "GET",
            url: '<?php echo env('APP_URL').'api/category/'; ?>'+id,
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "type":"web",
            },
            ...
            success: function(data) {
                ...
                { 
                    ...
                    $("#form_type").val(data.Data.type).trigger('change');
                    $("#form_id").val(id);
                }
            }
        });
    ...
    function editData() {
       ...
       $.ajax({
            type: "PUT",
            url: '<?php echo env('APP_URL').'api/editCategory'; ?>',
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "email":"<?php echo session('email'); ?>",
                "token":"<?php echo session('token'); ?>",
    ...
    function deleteData() {
       ...
       $.ajax({
            type: "DELETE",
            url: '<?php echo env('APP_URL').'api/delCategory'; ?>',
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "email":"<?php echo session('email'); ?>",
                "token":"<?php echo session('token'); ?>",
