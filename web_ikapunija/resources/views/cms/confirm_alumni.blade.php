            ...
            <div class="modal-body">
                <form id="form_submit"> 
                    ...
                    <div class="form-group ves-custom" id="ves_delete" style="font-size: 1.3rem; text-align: center; margin-bottom: -10px;"> 
                        <label>Are You Sure To Approve This Data ?</label>
                    </div>
                    <div class="form-group ves-custom" id="ves_judul">
                        <label>Judul</label>
                        <input type="text" class="form-control form-custom" id="form_judul" placeholder="Enter Judul">
                    </div>

                    <div class="form-group ves-custom" id="ves_review_singkat">
                        <label>Review Singkat</label>
                        <input type="text" class="form-control form-custom" id="form_review_singkat" placeholder="Enter Review Singkat">
                    </div>
                    ...
                    <div class="form-group ves-custom" id="ves_priority">
                        ...
                        <input type="number" class="form-control form-custom" id="form_priority" placeholder="Enter Priority">
                    </div>
                </form>
            </div>
...
<script>
            ...
            columns: [
                ...
                {data: 'prodi_name'},
                {
                    ...
                    render:function(data, type, row){
                        return ''+
                            '<a  data-toggle="modal" data-target="#modal-crud" href="#" onclick="approveModal(this)" data-id="'+row.id+'" style="margin: 0 5px;">'+
    ...
    function addData() {
       ...
       $.ajax({
            type: "POST",
            url: '<?php echo env('APP_URL').'api/addUserAlumni'; ?>',
            headers: {
            ...
            success: function(data) {
                if(data.Error != null && data.StatusCode == null)
    ...
    function getDetail(obj) {
        ...
        $.ajax({
            type: "GET",
            url: '<?php echo env('APP_URL').'api/user_alumni/'; ?>'+id,
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "type":"web",
            },
            ...
            success: function(data) {
                ...
                {   
                    ...
                    $("#ves_fotoImg").data('images', '<?php echo $url."user_alumni/"; ?>'+data.Data.foto);
                }
            }
        });
    }
    ...
    function editData() {
       ...
       $.ajax({
            type: "PUT",
            url: '<?php echo env('APP_URL').'api/editUserAlumni'; ?>',
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "email":"<?php echo $_SESSION['email']; ?>",
                "token":"<?php echo $_SESSION['token']; ?>",
    ...
    function deleteData() {
       ...
       $.ajax({
            type: "DELETE",
            url: '<?php echo env('APP_URL').'api/delUserAlumni'; ?>',
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "email":"<?php echo $_SESSION['email']; ?>",
                "token":"<?php echo $_SESSION['token']; ?>",
