    ...
    <div class="modal fade" id="modal-crud">
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-header">
                <h4 id="modal-title" class="modal-title">Agenda</h4>
            ...
            <div class="modal-body">
                <form id="form_submit"> 
                    ...
                    <div class="form-group ves-custom" id="ves_priority">
                        <label class="d-flex">Priority <div class="cus_req" id="req_foto">&nbsp*</div></label>
...
<script>
    ...
    function addData() {
       ...
       $.ajax({
            type: "POST",
            url: '<?php echo env('APP_URL').'api/addAgenda'; ?>',
            headers: {
    ...
    function getDetail(obj) {
        ...
        $.ajax({
            type: "GET",
            url: '<?php echo env('APP_URL').'api/agenda/'; ?>'+id,
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "type":"web",
            },
            ...
            success: function(data) {
                ...
                { 
                    ...
                    $("#form_judul").val(detail.judul);
                    $("#form_id").val(id);
                    $("#form_review_singkat").val(detail.review_singkat);
                    $("#form_isi").summernote("code", detail.isi);
                    $("#form_category").val(detail.category).trigger('change');
                    $("#form_priority").val(detail.priority);
                    $("#ves_fotoImg").data('images', '<?php echo $url."agenda/"; ?>'+detail.foto);
                }
            }
        });
    }
    ...
    function editData() {
       ...
       $.ajax({
            type: "PUT",
            url: '<?php echo env('APP_URL').'api/editAgenda'; ?>',
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "email":"<?php echo session('email'); ?>",
                "token":"<?php echo session('token'); ?>",
    ...
    function deleteData() {
       ...
       $.ajax({
            type: "DELETE",
            url: '<?php echo env('APP_URL').'api/delAgenda'; ?>',
            headers: {
                "apiToken":"<?php echo env('APPS_KEY'); ?>",
                "email":"<?php echo session('email'); ?>",
                "token":"<?php echo session('token'); ?>",
